      ...
      <section id="markt" className="container-x pb-20">
        ...
        <div className="sticky top-14 z-30 -mx-5 mb-6 border-y border-black/[0.06] bg-canvas/85 px-5 py-3 backdrop-blur-xl sm:-mx-8 sm:px-8">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <div className="-mx-1 flex items-center gap-2 overflow-x-auto px-1 pb-0.5 lg:mx-0 lg:overflow-x-visible lg:px-0">
              <div className="flex shrink-0 rounded-full bg-black/[0.04] p-1">
                {FUELS.map((f) => (
                  <button
                    key={f}
                    onClick={() => setFuel(f)}
              ...
              <input
                ...
                placeholder="max €"
                className="input h-10 w-24 shrink-0 px-3 text-[14px]"
              />
              <select
                value={sort}
                onChange={(e) => setSort(e.target.value as (typeof SORTS)[number])}
                className="input h-10 w-auto shrink-0 px-3 text-[14px]"
              >
                {SORTS.map((s) => (
